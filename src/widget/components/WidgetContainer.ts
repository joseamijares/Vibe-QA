...
function createDetailsForm(config: Required<VibeQAWidgetConfig>, state: WidgetState): HTMLElement {
  ...
  screenshotContainer.className = 'vibeqa-screenshot-modes';

  const screenshotBtn = document.createElement('button');
  screenshotBtn.className = 'vibeqa-media-button has-dropdown';
  ...
  screenshotDropdown.className = 'vibeqa-screenshot-dropdown';
  screenshotDropdown.setAttribute('data-open', 'false');

  const screenshotModes = [
    {
      mode: 'fullpage',
      icon: getFullPageIcon(),
      title: 'Full Page',
      desc: 'Capture entire page',
    },
    {
      mode: 'area',
      icon: getAreaIcon(),
  ...
  screenshotModes.forEach((modeInfo) => {
    ...
    modeOption.className = 'vibeqa-screenshot-mode';
    modeOption.setAttribute('data-screenshot-mode', modeInfo.mode);
    modeOption.innerHTML = `
      <span class="vibeqa-screenshot-mode-icon">${modeInfo.icon}</span>
      <div class="vibeqa-screenshot-mode-text">
        <div class="vibeqa-screenshot-mode-title">${modeInfo.title}</div>
        <div class="vibeqa-screenshot-mode-desc">${modeInfo.desc}</div>
      </div>
    `;
    screenshotDropdown.appendChild(modeOption);
  });

  screenshotContainer.appendChild(screenshotDropdown);
  mediaButtons.appendChild(screenshotContainer);
...
function getFullPageIcon(): string {
  return `<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 17V7m0 10l-3-3m3 3l3-3m5 3V7m0 10l-3-3m3 3l3-3" />
